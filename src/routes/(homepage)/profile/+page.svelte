			...
			<div class="flex-1 space-y-4">
				...
				<div class="flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-6">
					...
					<div class="flex gap-2">
						<a href="/profile/new_profile_picture"><button class="rounded-lg bg-zinc-800 px-4 py-1.5 text-sm text-white hover:bg-zinc-700 transition">
							Edit profile
						</button></a>
						<form action="/logout?/logout" method="POST">
								<button type="submit" class="rounded-lg bg-zinc-800 px-4 py-1.5 text-sm text-white hover:bg-zinc-700 transition">Log Out</button>
						</form>
						
						<button class="rounded-full bg-zinc-800 p-2 hover:bg-zinc-700 transition">
							<svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
								<a href="admin/articles_management"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"/></a>
							</svg>
						</button>
					</div>
				</div>
